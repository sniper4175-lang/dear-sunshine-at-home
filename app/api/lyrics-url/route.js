...
function isExamplePath(value) {
    return (
        String(value || '').toLowerCase().endsWith('/example.png') ||
        String(value || '').toLowerCase() === 'example.png'
    );
}
...
export async function GET(request) {
    try {
        ...
        if (songError) {
            console.error('lyrics-url song error:', songError);
            return NextResponse.json(
                { error: '가사지 정보를 확인하지 못했습니다.' },
                { status: 500 }
            );
        }
        ...
        try {
            ...
            console.error('lyrics-url program access error:', error);
            return NextResponse.json(
        ...
        if (
            ...
            return NextResponse.json(
                { error: `${song.program} 수강 회원만 이용할 수 있는 가사지입니다.` },
                { status: 403 }
            );
        }
